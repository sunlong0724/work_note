...
https://blog.csdn.net/qq_40179458/article/details/108852594
...
2. 
    +--------------+-------------------+-----------------------+-------------+  
    | Basic Header**1,2,3bytes**| Message Header**11,7,3,0bytes**| Extended Timestamp**0,4bytes** |  Chunk Data  |  
    +--------------+-------------------+-----------------------+--------------+  
    ...
    3. Extended Timestamp：扩展时间戳。长度可以是0或4字节。**只有在timestamp或timestamp delta用3个字节表示放不下时(0XFFFFFF)才启用该字段。**
    4. Chunk Data（块数据）  
        用户层面上真正想要发送的与协议无关的数据，长度在(0,chunkSize]之间。**chunkSize默认值是128.**
        实际音视频数据及协议控制消息值都在 Chunk Data中。

    5. 
        ![图片](../assest/rtmp_message_full.png)
...
4. **rtmp chunkdata 携带h264数据，其实就是flv tag data携带的h264数据**  
    ...
    3. AVCPacketType 是 8 个 bit。取值 0：表示 Composition Time 后面的数据是 **AVCDecorderConfigurationRecord 结构参考flv video tag data**，如图一所示；取值 1：表示 Composition Time 后面的数据是 H264 NALU 格式，如图二所示。
...
# RTMP 创建流的基本流程 (区分协议规定的情况和实际应用时的情况)
https://blog.csdn.net/qq_40179458/article/details/108852594

![图片](../assest/rtmp_sop.png)
![图片](../assest/rtmp_real_handshake.png)
![图片](../assest/rtmp_real_connect.png)
![图片](../assest/rtmp_real_publish.png)
![图片](../assest/rtmp_real_play.png)